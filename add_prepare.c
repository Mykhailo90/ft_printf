...
{
	if (com->pr && ft_atoi(com->pr) == 0 && !input_symb(com->flags, '#'))
		str = add_hesh(com, str, ch);
	if (!com->pr && com->width && ft_atoi(com->width) > (int)ft_strlen(str) &&
...
char					*add_esp_for_c(char *str, t_list *com)
{
	...
	if (str[0] == 0 && com->specificator == 'c')
		len_ch = 1;
	width = ft_atoi(com->width);
	flag = width - len_ch;
	tmp = ft_strnew(width - len_ch);
	if (com->pr && ft_atoi(com->pr) == 0 && !com->width)
		flag++;
	while (i < flag)
		tmp[i++] = ' ';
